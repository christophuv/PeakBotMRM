...
def validateExperiment(expName, targetFile, curatedPeaks, samplesPath, modelFile, 
    ...
    for i, metric in enumerate(rows):
        ...
        plt.tight_layout()
        fig.savefig(os.path.join(expDir,"fig_Metric_%s.png"%(metric)), dpi = 300)
        plt.close()
    ...
    plt.setp(ax.get_xticklabels(), rotation=45, ha="right",rotation_mode="anchor")
    plt.tight_layout()
    fig.savefig(os.path.join(expDir,"fig_Metrics.png"), dpi = 300)
    plt.close()
    ...
    plt.setp(ax.get_xticklabels(), rotation=45, ha="right",rotation_mode="anchor")
    plt.tight_layout()
    fig.savefig(os.path.join(expDir,"fig_Metrics_short.png"), dpi = 300)
    plt.close()
    
    with open(os.path.join(expDir, "pd_individualInstances.pickle"), "wb") as fout:
    ...
    p9.ggsave(plot=plot, filename=os.path.join(expDir, "fig_comparisonManualIntegration1.png"), width=10, height=10, dpi=300)
    ...
    p9.options.figure_size = (10,10)
    p9.ggsave(plot=plot, filename=os.path.join(expDir, "fig_comparisonManualIntegration2.png"), width=10, height=10, dpi=300)
    ...
    p9.options.figure_size = (10,10)
    p9.ggsave(plot=plot, filename=os.path.join(expDir, "fig_comparisonIntegration1.png"), width=10, height=10, dpi=300)
